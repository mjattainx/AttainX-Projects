...
function Table({ columns, data, updateMyData, skipReset, setData, resetData, renderSubmit, loadingSubmit, loadingGet }) {
  ...
  return (
    ...
    <div className="crud-ui-bar">
      ...
      <CsvDownload className = "ui labeled icon button" data={data} filename = "CertificatesTable.csv"><i className = "download icon"></i> Download as CSV File</CsvDownload>
      <br/>
    </div>
      {!loadingGet ?
      <table {...getTableProps()}>
...
const CertsTable2Edit = () => {
  ...
  const renderSubmit = async () => {
    ...
    await axios.put('https://pwnk7rnvjl.execute-api.us-east-1.amazonaws.com/dev', payload, {
      headers : {
        "Content-Type": "application/json",
        "Accept": "application/json"
      }
    ...
    // await axios.post('https://pwnk7rnvjl.execute-api.us-east-1.amazonaws.com/dev', payloadCreate, {
    //   headers : {
    //     "Content-Type": "application/json",
    //     "Accept": "application/json"
    //   }
...
//     let one =
//     "https://pwnk7rnvjl.execute-api.us-east-1.amazonaws.com/dev";
  

//   const requestOne = await axios.post(one, payloadCreate,    {headers : {
//     "Content-Type": "application/json",
//     "Accept": "application/json"
//   }});
//   const requestTwo = await axios.put(one, payloadUpdate,    {headers : {
//     "Content-Type": "application/json",
//     "Accept": "application/json"
//   }});
//   const requestThree = await axios.delete(one, payloadDelete, 
//     {headers : {
// "Content-Type": "application/json",
// "Accept": "application/json"
// }})
  ...
  const addFillerData = () => {
    for (var k in Array.from({length: 100}, (x, i) => i)) {
      const newData = {
        ...
        'certificate' : (parseInt(k) + 1).toString(),
        'provider' : (parseInt(k) + 1).toString(),
        'effectiveDate' : moment(new Date(+(new Date()) - Math.round(Math.random()*10000000000)))
        .format('MM/DD/YYYY'),
        'expirationDate' : moment(new Date(+(new Date()) - Math.round(Math.random()*1000000000)))
        .format('MM/DD/YYYY'),
      }
  ...
  return (
    <Styles>
      {/* <div>
        <button className = "ui labeled icon button" onClick={resetData}> <i className = "sync alternate icon"></i>Reset Data</button>      
        <button className = "ui labeled icon button" onClick={() => renderSubmit()}> <i className = "paper plane icon"></i> Submit Changes </button>
      </div> */}
      {/* <div>
        debugging
        <button onClick={() => console.log(JSON.stringify(data))}>data</button>
...
export default CertsTable2Edit